!>\file unit_conv_scheme_1.F90
!! This file contains a unit_conv_scheme_1 CCPP scheme that does nothing
!! except requesting the minimum, mandatory variables.

module unit_conv_scheme_1
  ...
  private
  public :: unit_conv_scheme_1_run

  ! This is for unit testing only
  real(kind=kind_phys), parameter, dimension(1:2) :: target_values = (/-1.0_kind_phys, 1.0_kind_phys/)
  real(kind=kind_phys), parameter, dimension(1:2) :: target_values2 = (/-42.0_kind_phys, 42.0_kind_phys/)

contains

  !! \section arg_table_unit_conv_scheme_1_run Argument Table
  !! \htmlinclude unit_conv_scheme_1_run.html
  !!
  subroutine unit_conv_scheme_1_run(instance, data_array, data_array2, data_array_opt, errmsg, errflg)
    ...
    write(error_unit, '(a,e12.4)') &
        'In unit_conv_scheme_1_run: checking min/max values of data array to be approximately ', &
    ...
    if (abs(minval(data_array) - target_values(instance)) > 0.01_kind_phys .or. &
      ...
      write(errmsg, '(3(a,e12.4),a)') &
          "Error in unit_conv_scheme_1_run, expected values for data_array of approximately ", &
    ...
    write(error_unit, '(a,e12.4)') &
        'In unit_conv_scheme_1_run: checking min/max values of data array 2 to be approximately ', &
    ...
    if (abs(minval(data_array2) - target_values2(instance)) > 0.01_kind_phys .or. &
      ...
      write(errmsg, '(3(a,e12.4),a)') &
          "Error in unit_conv_scheme_1_run, expected values for data array 2 of approximately ", &
    ...
    if (instance==1) then
      if (.not. present(data_array_opt)) then
        write(errmsg, '(a)') 'Error in unit_conv_scheme_1_run, optional data array expected but not present'
        write(errmsg, '(a,i0)') 'for instance ', instance
        errflg = 1
        return
      end if
      write(error_unit, '(a,e12.4)') &
          'In unit_conv_scheme_1_run: checking min/max values of optional data array to be approximately ', target_values(instance)
      ...
      write(errmsg, '(a)') 'Error in unit_conv_scheme_1_run, optional data array not expected but present'
      write(errmsg, '(a,i0)') 'for instance ', instance
      errflg = 1
      return
    end if

  end subroutine unit_conv_scheme_1_run

end module unit_conv_scheme_1
