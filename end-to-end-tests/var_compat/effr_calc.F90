...
contains
  ...
  subroutine effr_calc_run(ncol, nlev, effrr_in, effrg_in, ncg_in, nci_out, &
    ...
    real(kind=kind_phys), intent(in) :: effrr_in(:, :)
    real(kind=kind_phys), intent(in), optional :: effrg_in(:, :)
    real(kind=kind_phys), intent(in), optional :: ncg_in(:, :)
    real(kind=kind_phys), intent(out), optional :: nci_out(:, :)
    real(kind=kind_phys), intent(inout) :: effrl_inout(:, :)
    real(kind=kind_phys), intent(out), optional :: effri_out(:, :)
    real(kind=8), intent(inout) :: effrs_inout(:, :)
    logical, intent(in) :: has_graupel
    real(kind=kind_phys), intent(inout) :: scalar_var
    character(len=512), intent(out) :: errmsg
    integer, intent(out) :: errflg
    real(kind=kind_phys), intent(out), optional :: ncl_out(:, :)
    real(kind=kind_phys), intent(inout) :: tke_inout
    real(kind=kind_phys), intent(inout) :: tke2_inout

    !----------------------------------------------------------------

    real(kind=kind_phys), parameter :: re_qc_min = 2.5 ! microns
    real(kind=kind_phys), parameter :: re_qc_max = 50. ! microns
    real(kind=kind_phys), parameter :: re_qi_avg = 75. ! microns
    real(kind=kind_phys) :: effrr_local(ncol, nlev)
    real(kind=kind_phys) :: effrg_local(ncol, nlev)
    real(kind=kind_phys) :: ncg_in_local(ncol, nlev)
    real(kind=kind_phys) :: nci_out_local(ncol, nlev)
