...
contains
  ...
  !! \section arg_table_opt_arg_scheme_timestep_final Argument Table
  !! \htmlinclude opt_arg_scheme_timestep_finalize.html
  !!
  subroutine opt_arg_scheme_timestep_final(nx, var, opt_var, opt_var_2, errmsg, errflg)
    character(len=*), intent(out) :: errmsg
    integer, intent(out) :: errflg
    integer, intent(in) :: nx
  ...
  end subroutine opt_arg_scheme_timestep_final
