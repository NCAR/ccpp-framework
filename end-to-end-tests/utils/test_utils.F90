...
contains
  logical function check_list(test_list, chk_list, list_desc, suite_name)
    ...
    ! DH*
    write(0,*) ''
    do lindex = 1, size(chk_list)
      write(errmsg, '(a,i4,1x,a)') 'Debug:  chk_list', lindex, trim(chk_list(lindex))
      write(0,*) trim(errmsg)
    end do
    do lindex = 1, size(test_list)
      write(errmsg, '(a,i4,1x,a)') 'Debug: test_list', lindex, trim(test_list(lindex))
      write(0,*) trim(errmsg)
    end do
    ! *DH
