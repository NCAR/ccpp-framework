...
_CCPP_FILES = _UTILITY_FILES + \
              [os.path.join(_BUILD_DIR, "ccpp", "test_host_ccpp_cap.F90"),
               ...
               os.path.join(_BUILD_DIR, "ccpp", "ccpp_temp_suite_cap.F90")]
_DEPENDENCIES = [os.path.join(_TEST_DIR, "adjust", "temp_kinds.F90"),
...
class TestCapgenHostDataTables(unittest.TestCase, BaseTests.TestHostDataTables):
    database = _DATABASE
...
class CommandLineCapgenHostDatafileRequiredFiles(unittest.TestCase, BaseTests.TestHostCommandLineDataFiles):
    database = _DATABASE
    host_files = _HOST_FILES
    suite_files = _SUITE_FILES
    utility_files = _UTILITY_FILES
...
class TestCapgenTempSuite(unittest.TestCase, BaseTests.TestSuiteExcludeProtected):
    database = _DATABASE
...
class CommandLineCapgenTempSuite(unittest.TestCase, BaseTests.TestSuiteExcludeProtectedCommandLine):
    database = _DATABASE
    required_vars = _REQUIRED_VARS_TEMP + _PROT_VARS_TEMP
    input_vars = _INPUT_VARS_TEMP + _PROT_VARS_TEMP
    required_vars_excluding_protected = _REQUIRED_VARS_TEMP
