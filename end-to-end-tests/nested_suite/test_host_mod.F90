...
contains
  ...
  logical function compare_data()
    ...
    integer, parameter :: lifecycle_counter_expected = 2
    ...
    if (lifecycle_counter /= lifecycle_counter_expected) then
      write(6, '(a,i0,a,i0)') &
          'Error: lifecycle_counter does not match expected value: ', lifecycle_counter, ' vs ', lifecycle_counter_expected
      compare_data = .false.
    end if
