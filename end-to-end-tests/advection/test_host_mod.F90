module test_host_mod
  ...
  real(kind=kind_phys), private, allocatable :: check_vals(:, :, :)
  real(kind=kind_phys), private :: check_temp(ncols, pver)
...
contains
  ...
  subroutine init_data(constituent_array, index_qv_use, index_liq, index_ice, index_dyn)
    ...
    ! Dummy arguments
    real(kind=kind_phys), pointer :: constituent_array(:, :, :) ! From host & suites
    ...
    integer :: cind
    integer :: itime
    real(kind=kind_phys) :: qmax
    real(kind=kind_phys), parameter :: inc = 0.1_kind_phys
