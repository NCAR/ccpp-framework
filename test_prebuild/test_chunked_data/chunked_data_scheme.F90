...
contains
  ...
  subroutine chunked_data_scheme_run(nchunk, data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(2(a,i3))') 'In chunked_data_scheme_run: checking size of data array for chunk', nchunk, &
        ' to be', data_array_sizes(nchunk)
  ...
  subroutine chunked_data_scheme_timestep_finalize(data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(a,i3)') 'In chunked_data_scheme_timestep_finalize: checking size of data array to be', sum(&
        data_array_sizes)
    if (size(data_array)/=sum(data_array_sizes)) then
      write(errmsg, '(2(a,i3))') "Error, expected size(data_array)==", sum(data_array_sizes), "but got ", size(&
          data_array)
  ...
  subroutine chunked_data_scheme_finalize(data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(a,i3)') 'In chunked_data_scheme_finalize: checking size of data array to be', sum(&
        data_array_sizes)
    if (size(data_array)/=sum(data_array_sizes)) then
      write(errmsg, '(2(a,i3))') "Error, expected size(data_array)==", sum(data_array_sizes), "but got ", size(&
          data_array)
