...
contains
  ...
  subroutine blocked_data_scheme_timestep_init(data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(a,i3)') 'In blocked_data_scheme_timestep_init: checking size of data array to be', sum(&
        data_array_sizes)
    if (size(data_array)/=sum(data_array_sizes)) then
      write(errmsg, '(2(a,i3))') "Error, expected size(data_array)==", sum(data_array_sizes), " but got ", size(&
          data_array)
  ...
  subroutine blocked_data_scheme_run(nb, data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(2(a,i3))') 'In blocked_data_scheme_run: checking size of data array for block', nb, &
        ' to be', data_array_sizes(nb)
  ...
  subroutine blocked_data_scheme_timestep_finalize(data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(a,i3)') 'In blocked_data_scheme_timestep_finalize: checking size of data array to be', sum(&
        data_array_sizes)
    if (size(data_array)/=sum(data_array_sizes)) then
      write(errmsg, '(2(a,i3))') "Error, expected size(data_array)==", sum(data_array_sizes), "but got ", size(&
          data_array)
  ...
  subroutine blocked_data_scheme_finalize(data_array, errmsg, errflg)
    ...
    errflg = 0
    ! Check size of data array
    write(error_unit, '(a,i3)') 'In blocked_data_scheme_finalize: checking size of data array to be', sum(&
        data_array_sizes)
    if (size(data_array)/=sum(data_array_sizes)) then
      write(errmsg, '(2(a,i3))') "Error, expected size(data_array)==", sum(data_array_sizes), "but got ", size(&
          data_array)
