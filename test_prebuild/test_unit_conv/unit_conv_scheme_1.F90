...
module unit_conv_scheme_1
   ...
   real(kind_phys), parameter :: target_value = 1.0_kind_phys
...
!!
   subroutine unit_conv_scheme_1_run(data_array, data_array2, data_array_opt, errmsg, errflg)
      ...
      if (minval(data_array)<0.99*target_value .or. maxval(data_array)>1.01*target_value) then
         write(errmsg,'(3(a,e12.4),a)') "Error in unit_conv_scheme_1_run, expected values for data_array of approximately ", &
      ...
      if (minval(data_array2)<0.99*target_value2 .or. maxval(data_array2)>1.01*target_value2) then
         write(errmsg,'(3(a,e12.4),a)') "Error in unit_conv_scheme_1_run, expected values for data array 2 of approximately ", &
