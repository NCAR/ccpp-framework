...
| `--kind-type` | no (repeatable) | Fortran kind mappings, e.g. `kind_phys=REAL64` |
...
**`ccpp_kinds.F90`**:
- Simple Fortran module `ccpp_kinds` containing `use` statements that import all kind
  parameters specified via `--kind-type`
- Makes kind parameters available to both schemes and caps without circular dependencies
