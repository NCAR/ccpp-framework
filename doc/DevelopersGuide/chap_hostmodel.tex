...
As of now, the CCPP physics library and software framework are built as part of the host model (\execout{SCM}, \execout{FV3}). \execout{SCM} uses a \execout{cmake} build system for both the library and the software framework, \execout{FV3} a traditional \execout{make} build system for the library and a \execout{cmake} build system for the software framework. Accordingly, \execout{CMakeLists.txt} files in the \execout{ccpp-physics} directory tree refer to an \execout{SCM} build, while \execout{makefile} files refer to an \execout{FV3} build. Work is underway to provide a universal build system based on \execout{cmake} that can be used with all host models: \execout{SCM}, \execout{FV3}, \dots

In addition, the current build systems do not make full use of the makefile fragments auto-generated by \execout{ccpp\_prebuild.py} (c.\,f. previous section): While \execout{SCM} uses hardcoded lists of physics schemes and auto-generated physics scheme caps, \execout{FV3} makes use of the auto-generated list of physics scheme cups but uses a hardcoded list of physics scheme files. This is also due to \execout{ccpp\_prebuild.py} at the moment only producing traditional \execout{makefile} fragments (e.\,g. \execout{CCPP\_SCHEMES.mk} and \execout{CCPP\_CAPS.mk}). Again, work is underway to create include files suitable for \execout{cmake} for both schemes and caps, and to integrate these into the build system.
\subsection{Build steps}\label{sec_ccpp_build_steps}
The instructions to build the library and software framework use the \execout{cmake} build system that is also used with the GMTB single column model \execout{SCM}. Several steps are required in the following order:
\begin{description}
