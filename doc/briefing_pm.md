# capgen-ng — Briefing for Project Management
...
**`capgen-ng`** is a third generator, started in early May 2026,
...
mechanism — the central feature capgen-ng inherited from capgen —
...
`capgen-ng` was multi-instance **from day one**: every constituent
...
`capgen-ng` is moving `diagnostic_name` (and a handful of other
...
## 4. What `capgen-ng` does better than capgen — at any scale
...
## 5. Additional features of `capgen-ng` compared to `capgen`

Features that exist only in capgen-ng (some exist in prebuild):
...
capgen-ng decisions.  capgen-ng is structured so all three
